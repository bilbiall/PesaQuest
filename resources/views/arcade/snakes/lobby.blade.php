...
<head>
    ...
    <style>
        ...
        .sc-accordion summary { list-style:none; cursor:pointer; user-select:none; }
        .sc-accordion summary::-webkit-details-marker { display:none; }
        .sc-accordion summary .chevron { transition:transform .15s; }
        .sc-accordion[open] summary .chevron { transform:rotate(180deg); }
    </style>
</head>
<body class="min-h-screen">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <details class="sc-card sc-accordion mb-6"{{ request('join') ? ' open' : '' }}>
            ...
            <div class="px-5 pb-5 grid sm:grid-cols-2 gap-4">
                <div>
                    <p class="text-xs font-bold mb-3 text-gray-300">Create a match</p>
                    <form method="POST" action="{{ route('arcade.snakes.match.create') }}" class="space-y-2" onsubmit="phTrack('pesatrail_match_create')">
                        @csrf
                        <input name="name" maxlength="40" placeholder="Name your match (optional)" class="sc-input">
                ...
                <div>
                    <p class="text-xs font-bold mb-3 text-gray-300">Join with a code</p>
                    <form method="POST" action="{{ route('arcade.snakes.match.join') }}" class="space-y-2" onsubmit="phTrack('pesatrail_match_join', {via:'code'})">
                        @csrf
        ...
        <div class="sc-card p-5 mb-6">
            ...
            <div class="space-y-2">
                ...
                <form method="POST" action="{{ route('arcade.snakes.match.join') }}" class="flex items-center justify-between gap-3 p-3 rounded-xl" style="background:rgba(255,255,255,.02);" onsubmit="phTrack('pesatrail_match_join', {via:'list'})">
                    @csrf
                    <input type="hidden" name="match_id" value="{{ $match->id }}">
                    <span class="text-xs text-gray-400">{{ $match->name ?: 'Match #' . $match->id }} — {{ $match->sessions_count }}/{{ $match->max_players }} players</span>
        ...
        <details class="sc-card sc-accordion mb-6" style="border-color:rgba(236,72,153,.3);">
            ...
            <div class="px-5 pb-5">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-bold mb-3 text-gray-300">Create a round</p>
                    ...
                    <div>
                        <p class="text-xs font-bold mb-3 text-gray-300">Join with a code</p>
                        <form method="POST" action="{{ route('arcade.snakes.wager.join') }}" class="space-y-2" onsubmit="phTrack('pesatrail_wager_join', {via:'code'})">
                            @csrf
        ...
        <div class="sc-card p-5">
            ...
            <div class="space-y-2">
                ...
                <form method="POST" action="{{ route('arcade.snakes.wager.join') }}" class="flex items-center justify-between gap-3 p-3 rounded-xl" style="background:rgba(255,255,255,.02);" onsubmit="phTrack('pesatrail_wager_join', {via:'list'})">
                    @csrf
                    <input type="hidden" name="match_id" value="{{ $wm->id }}">
                    <span class="text-xs text-gray-400">{{ $wm->name ?: 'Round #' . $wm->id }} — KES {{ number_format($wm->stake_amount) }} entry — {{ $wm->sessions_count }}/{{ $wm->max_players }} players</span>
    ...
    <script>
        // No-ops if PostHog isn't configured (Admin → Analytics → Tracker Setup) —
