<div data-page-title="Life Story — PesaQuest">
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="relative pl-8">
        ...
        <div class="space-y-4 tl-chapter" id="tl-chapter-{{ $chapter }}">
            ...
            @endphp
            <div class="relative {{ $loop->index >= 5 ? 'tl-extra' : '' }}">
                {{-- Dot --}}
        ...
        @if($chapterEvents->count() > 5)
        <button type="button" class="tl-more-btn" data-target="tl-chapter-{{ $chapter }}" data-count="{{ $chapterEvents->count() - 5 }}">
            <span class="tl-more-arrow">▼</span> <span class="tl-more-label">Show {{ $chapterEvents->count() - 5 }} more event{{ $chapterEvents->count() - 5 === 1 ? '' : 's' }}</span>
        </button>
        @endif
    </div>
    @endforeach
...
<script>
document.querySelectorAll('.tl-more-btn').forEach(function (btn) {
    btn.addEventListener('click', function () {
        var target = document.getElementById(btn.dataset.target);
        var expanding = !target.classList.contains('tl-expanded');
        target.classList.toggle('tl-expanded', expanding);
        btn.querySelector('.tl-more-arrow').textContent = expanding ? '▲' : '▼';
        btn.querySelector('.tl-more-label').textContent = expanding
            ? 'Show fewer'
            : 'Show ' + btn.dataset.count + ' more event' + (btn.dataset.count === '1' ? '' : 's');
        if (!expanding) target.closest('.relative.pl-8').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });
});
</script>
</div>
