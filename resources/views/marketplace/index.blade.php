...
@media(max-width:767px){
    ...
    .hero-wrap{
        ...
        padding:0 1rem 1rem!important;gap:0!important;max-width:100%!important;
        position:relative!important;min-height:250px!important;
    }
    ...
    .city-img{width:210px!important;opacity:.7!important;}
    .city-glow{width:160px!important;height:160px!important;opacity:.35!important;}
    ...
    .hero-right{order:2!important;flex-direction:row!important;gap:.625rem!important;position:relative!important;z-index:2!important;margin-top:.625rem!important;}
    .hero-stat-card{flex:1;min-width:0;padding:.625rem .75rem!important;}
    .stat-card-img{width:32px;height:32px;}
    .hero-stat-value{font-size:.95rem!important;}
    .hero-title{font-size:1.6rem!important;line-height:1.15!important;margin-bottom:.5rem!important;}
    .hero-subtitle{font-size:.8rem;max-width:100%;margin-bottom:.625rem!important;}
    .hero-avail{font-size:.68rem;margin-bottom:.5rem;}
            ...
            <div>
                <div class="relative h-32 sm:h-40 overflow-hidden rounded-t-3xl" :class="'cat-' + inspecting.category">
                    ...
                    <template x-if="!inspecting.image">
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="icon-bob w-12 h-12 sm:w-16 sm:h-16" x-html="pqIcon(inspecting.icon || catEmoji(inspecting.category), 'w-12 h-12 sm:w-16 sm:h-16')"></span>
                        </div>
                    </template>
                    <button @click="inspecting=null;buyMsg='';"
                            class="absolute top-3 right-3 sm:top-4 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center"
                ...
                <div class="px-4 pt-2 pb-3 sm:px-6 sm:pt-2 sm:pb-4">
                    <div class="mb-4 sm:mb-5">
                        <h2 class="text-sm sm:text-xl font-black text-white" x-text="inspecting.name"></h2>
                        ...
                        <div class="p-3 sm:p-4 space-y-1.5 sm:space-y-2">
                            ...
                            <template x-if="inspecting.income > 0 || inspecting.cost > 0">
                                <div class="flex items-center justify-between text-xs sm:text-sm border-t border-white/10 pt-2 mt-2">
                                    ...
                                    <span class="font-black text-sm sm:text-lg" :class="inspecting.net >= 0 ? 'text-emerald-400' : 'text-red-400'"
                                          x-text="(inspecting.net >= 0 ? '+' : '') + 'Ksh ' + Math.abs(inspecting.net).toLocaleString() + '/mo'"></span>
                                </div>
                            </template>
                            <template x-if="inspecting.payback">
                                <div class="mt-3 rounded-xl px-2.5 py-2 sm:px-3 text-[11px] sm:text-xs font-bold text-emerald-400 flex items-start gap-1"
                                     style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);">
                                    <x-icon name="bolt" class="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 mt-0.5" />
                                    <span>Pays itself off in ~<span x-text="inspecting.payback"></span> game months
                                    (<span x-text="gdApprox(inspecting.payback * 30)"></span>), then pure profit</span>
                                </div>
                            </template>
                            <template x-if="!inspecting.payback && inspecting.projected && inspecting.rate > 0">
                                <div class="mt-3 rounded-xl px-2.5 py-2 sm:px-3 text-[11px] sm:text-xs font-bold text-indigo-300 flex items-start gap-1"
                                     style="background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.2);">
                                    <x-icon name="trend-up" class="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 mt-0.5" />
                                    <span>At <span x-text="inspecting.rate"></span>%/mo, worth ~Ksh <span x-text="(inspecting.projected||0).toLocaleString()"></span> in 12 game months (<span x-text="gdApprox(360)"></span>)</span>
                                </div>
                            </template>
                            <template x-if="inspecting.rate < 0">
                                <div class="mt-3 rounded-xl px-2.5 py-2 sm:px-3 text-[11px] sm:text-xs font-bold text-orange-300 flex items-start gap-1"
                                     style="background:rgba(251,146,60,.1);border:1px solid rgba(251,146,60,.2);">
                                    <x-icon name="warning" class="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 mt-0.5" />
                                    <span>Loses ~<span x-text="Math.abs(inspecting.rate)"></span>%/mo in value regardless of upkeep — maintaining it protects the income, not the price. Good for cash flow, not for growing net worth.</span>
                                </div>
                            </template>
                            <template x-if="inspecting.net < 0 && !inspecting.payback">
                                <div class="mt-3 rounded-xl px-2.5 py-2 sm:px-3 text-[11px] sm:text-xs font-bold text-orange-400 flex items-start gap-1"
                                     style="background:rgba(251,146,60,.1);border:1px solid rgba(251,146,60,.2);">
                                    <x-icon name="warning" class="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0 mt-0.5" />
                                    <span>This asset costs more than it earns. Only buy if you can absorb the monthly cost.</span>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    <template x-if="inspecting.afford_pct > 0">
                        <div class="mb-4 sm:mb-5 rounded-2xl overflow-hidden" style="border:1px solid rgba(255,255,255,.08);">
                            <div class="px-3 py-1.5 sm:px-4 sm:py-2" style="background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.06);">
                                <p class="text-[11px] sm:text-xs font-black text-white inline-flex items-center gap-1"><x-icon name="bar-chart" class="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Can you really afford this?</p>
                            </div>
                    ...
                    <template x-if="inspecting.bill">
                        <div class="mb-4 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 flex items-start gap-2"
                             style="background:rgba(251,146,60,.08);border:1px solid rgba(251,146,60,.2);">
                            <x-icon name="bolt" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-400 flex-shrink-0 mt-0.5" />
                            <p class="text-[11px] sm:text-xs text-orange-300 leading-snug">Buying this will automatically add a recurring bill to your expenses.</p>
                        </div>
                    </template>

                    <div class="mb-4 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 flex items-start gap-2"
                         style="background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.2);">
                        <x-icon name="bulb" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-400 shrink-0 mt-0.5" />
                    ...
                    <template x-if="inspecting.financing && !inspecting.maxed">
                        <div class="mb-4 rounded-2xl overflow-hidden" style="border:1px solid rgba(245,158,11,.25);">
                            ...
                            <div class="p-3 sm:p-4 space-y-1.5 text-xs sm:text-sm">
                    ...
                    <div x-show="buyMsg" x-cloak x-transition
                         class="mb-4 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-[11px] sm:text-xs font-bold text-center"
